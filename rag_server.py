...
async def populate_initial_data_async():
    ...
    collection_names_to_populate = {
        ...
        "general_instructions": [
            DocumentInput(
                text="Вопрос: Как узнать, какие курьеры сегодня на смене? Ответ: Для просмотра активных смен воспользуйтесь функцией 'Просмотр смен по складу' или обратитесь к вашему диспетчеру.",
                metadata={"source": "База знаний FAQ v1.0", "topic": "Смены курьеров"},
                doc_id="faq_shifts_today"
            ),
            DocumentInput(
                text="Вопрос: Что делать, если я забыл пароль от системы? Ответ: Для восстановления пароля обратитесь в техническую поддержку по внутреннему номеру 1234 или напишите на support@example.com.",
                metadata={"source": "База знаний FAQ v1.0", "topic": "Доступ к системе"},
                doc_id="faq_password_reset"
            ),
            DocumentInput(
                text="Инструкция по оформлению возврата товара от клиента: 1. Уточните причину возврата. 2. Проверьте состояние товара. 3. Оформите акт возврата в системе XYZ. 4. Передайте товар на склад возвратов.",
                metadata={"source": "Инструкции для директоров v2.1", "section": "Работа с клиентами"},
                doc_id="instr_client_returns"
            )
        ]
    }
