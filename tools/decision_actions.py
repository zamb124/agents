...
def take_action_on_courier(action_type: str, courier_id: str, reason: str, shift_id: str = None, warehouse_id: str = None) -> dict:
    ...
    Все действия логируюца. `warehouse_id` пока оставим, но для `delete_shift` он может быть не нужен если есть `shift_id`.
    """
    logger.info(f"[ACTION MOCK] Применяем действие: {action_type}, Курьер ID: {courier_id}, Причина: '{reason}', Смена ID: {shift_id}, Склад ID: {warehouse_id}")

    if courier_id not in MOCK_COURIERS_DB: # Провяряем, есть ли такой курьер ваще
    ...
    if action_type == "delete_shift":
        ...
        if shift_id: # Если указан конкретный ID смены
            if shift_id in MOCK_SHIFTS_DB and MOCK_SHIFTS_DB[shift_id]["courier_id"] == courier_id:
                if MOCK_SHIFTS_DB[shift_id]["status"] == "active" or MOCK_SHIFTS_DB[shift_id]["status"] == "planned":
                    ...
                    MOCK_SHIFTS_DB[shift_id]["status"] = "cancelled_by_support" # Меняем статус
        ...
        message = f"Неизвестный тип действия: {action_type}. Что-то пошло не так."
        action_successful = False
