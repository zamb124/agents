...
from .warehouse_api import get_warehouse_by_director_login, find_warehouse_by_name_or_id # Добавили find_warehouse_by_name_or_id
...
class GetWarehouseInput(BaseModel): # Для get_warehouse_by_director_login
    director_login: str = Field(description="Логин директора (Telegram username) для определения его склада.")

class FindWarehouseInput(BaseModel): # Новый класс для find_warehouse_by_name_or_id
    identifier: str = Field(description="ID или название (можно частичное) склада для поиска и проверки существования.")

# ... (TakeActionInput, QueryRAGInput, GetCourierShiftsInput без изменений) ...
...
class SearchCourierTool(BaseTool):
    name: str = "search_courier"
    description: str = (
        "Используется для поиска информации о курьере по его ID или ФИО. "
        "Крайне рекомендуется передавать `warehouse_id` для поиска по имени. "
        "Возвращает информацию о курьере, сообщение об ошибке или список кандидатов, если найдено несколько."
    )
    ...
    def _run(self, identifier: str, warehouse_id: Optional[str] = None, **kwargs) -> Dict[str, Any]:
        ...
        logger.info(f"[{self.name}._run] Вызван синхронно с: identifier='{identifier}', warehouse_id='{warehouse_id}'")
    ...
    async def _arun(self, identifier: str, warehouse_id: Optional[str] = None, **kwargs) -> Dict[str, Any]:
        ...
        logger.info(f"[{self.name}._arun] Вызван асинхронно с: identifier='{identifier}', warehouse_id='{warehouse_id}'")
        return self._run(identifier=identifier, warehouse_id=warehouse_id)

search_courier_tool = SearchCourierTool()


get_warehouse_by_login_tool = Tool.from_function( # Переименовал для ясности
    func=get_warehouse_by_director_login,
    name="get_warehouse_by_director_login",
    description="Используется для ПЕРВОНАЧАЛЬНОЙ попытки определения склада, к которому привязан директор, по его логину (например, telegram username).",
    args_schema=GetWarehouseInput
)

# --- Новый инструмент для поиска склада ---
class FindWarehouseTool(BaseTool):
    name: str = "find_warehouse_by_name_or_id"
    description: str = (
        "Используется для поиска и проверки существования склада по его ID или названию (можно частичному), "
        "когда автоматическое определение по логину директора не удалось или когда пользователь сам указал данные склада. "
        "Возвращает информацию о складе, сообщение об ошибке или список кандидатов, если найдено несколько."
    )
    args_schema: Type[BaseModel] = FindWarehouseInput

    async def _run(self, identifier: str, **kwargs) -> Dict[str, Any]:
        if kwargs: logger.warning(f"[{self.name}._run] Получены неожиданные kwargs: {kwargs}")
        logger.info(f"[{self.name}._run] Вызван синхронно с: identifier='{identifier}'")
        return await find_warehouse_by_name_or_id(identifier=identifier)

    async def _arun(self, identifier: str, **kwargs) -> Dict[str, Any]:
        if kwargs: logger.warning(f"[{self.name}._arun] Получены неожиданные kwargs: {kwargs}")
        logger.info(f"[{self.name}._arun] Вызван асинхронно с: identifier='{identifier}'")
        return await self._run(identifier=identifier)
...
class QueryKnowledgeBaseTool(BaseTool):
    ...
    def _run(self, query_text: str, collection_name: str, top_k: int = 3, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if loop.is_running():
                logger.error(f"[{self.name}._run] Обнаружен запущенный event loop. Такой синхронный вызов не сработает корректно для асинхронного инструмента.")
                ...
                return asyncio.run(self._arun(query_text=query_text, collection_name=collection_name, top_k=top_k, **kwargs))
...
collector_tools = [
    search_courier_tool,
    #get_warehouse_by_login_tool, # Переименовал
    find_warehouse_tool,         # Добавил новый
